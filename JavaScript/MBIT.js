...
console.log(`Welcome ${userName} to our MBTI Test!`);
console.log("Please answer each question with A or B.\n");
...
for (let i = 0; i < questions.length; i++) {
  ...
  while (true) {
    ...
    answer = prompt("Your answer (A/B): ").trim().toUpperCase();
    if (answer === "A" || answer === "B") break;
    console.log("Invalid choice. Please enter only A or B.\n");
  }
  responses[i] = answer;
}
...
for (let g = 0; g < groups.length; g++) {
  ...
  for (let q of groups[g]) {
    if (responses[q] === "A") {
      console.log("A. " + traits[q][0]);
      countA++;
    } else {
      console.log("B. " + traits[q][1]);
      countB++;
    }
  }
